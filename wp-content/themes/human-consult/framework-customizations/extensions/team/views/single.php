...
while ( have_posts() ) : the_post(); ?>
	<div class="col-md-5">
		<div class="vertical-item with_shadow content-padding overflow-hidden text-center">
			<?php the_post_thumbnail('human-consult-team-member'); ?>
			<div class="item-content">
				<?php the_title( '<h5 class="medium margin_0">', '</h5>' ); ?>
				<?php if ( ! empty( $atts['position'] ) ) : ?>
					<p class="small-text highlight margin_0"><?php echo wp_kses_post( $atts['position'] ); ?></p>
				<?php endif; //position ?>
					...
					<ul class="nav nav-tabs" role="tablist">
						...
						if ( ! empty( $atts['bio'] ) ) :
							...
							<li class="<?php echo ( $tab_num === 0 ) ? 'active' : '' ?>">
								<a href="#tab-<?php echo esc_attr( $unique_id . '-' . $tab_num ); ?>" role="tab"
								   data-toggle="tab">
									<?php esc_html_e( 'Biography', 'human-consult' ); ?>
								</a>
							</li>
							<?php
							$tab_num ++;
						...
						if ( ! empty( $atts['skills'] ) ) :
							...
							<li class="<?php echo ( $tab_num === 0 ) ? 'active' : '' ?>">
								<a href="#tab-<?php echo esc_attr( $unique_id . '-' . $tab_num ); ?>" role="tab"
								   data-toggle="tab">
									<?php esc_html_e( 'Skills', 'human-consult' ); ?>
								</a>
							</li>
							<?php
							$tab_num ++;
						...
						if ( ! empty( json_decode( $atts['form']['json'] )[1] ) ) :
							...
							<li class="<?php echo ( $tab_num === 0 ) ? 'active' : '' ?>">
								<a href="#tab-<?php echo esc_attr( $unique_id . '-' . $tab_num ); ?>" role="tab"
								   data-toggle="tab">
									<?php esc_html_e( 'Send Message', 'human-consult' ); ?>
								</a>
							</li>
							<?php
							$tab_num ++;
