...
if ( ! function_exists( 'human_consult_google_font_url' ) ) :
	function human_consult_google_font_url() {
		$fonts_url = '';
		$fonts     = array();
...
if ( ! function_exists( 'human_consult_add_font_styles_in_head' ) ) :
	function human_consult_add_font_styles_in_head() {
	...
	} //human_consult_add_font_styles_in_head()
...
if ( ! function_exists( 'human_consult_return_comments_walker' ) ) :
	function human_consult_return_comments_walker() {
		return new Human_Consult_Comments_Walker;
	}
endif;


if ( ! function_exists( 'human_consult_the_attached_image' ) ) :
	/**
	 * Print the attached image with a link to the next attached image.
	 */
	function human_consult_the_attached_image() {
		$post = get_post();
		/**
		 ...
		 * }
		 */
		$attachment_size     = apply_filters( 'human_consult_attachment_size', array( 810, 810 ) );
	...
	} //human_consult_the_attached_image()

endif;

if ( ! function_exists( 'human_consult_list_authors' ) ) :
	/**
	 * Print a list of all site authors who published at least one post.
	 */
	function human_consult_list_authors() {
		...
		foreach ( $author_ids as $author_id ) :
			...
			?>
			<div class="author-meta side-item content-padding with_shadow overflow-hidden">
				<div class="row">
					<div class="col-md-4">
					...
					<div class="col-md-8">
						<div class="item-content">
							...
							if ( $twitter_url || $facebook_url || $google_plus_url ) :
								?>
								<span class="author-social greylinks">
									<?php if ( $twitter_url ) : ?>
			...
			<?php
		endforeach;
	} //human_consult_list_authors()

endif;

if ( ! function_exists( 'human_consult_get_columns_classes' ) ) :
	/**
	 * Define a sidebar position for manage main column CSS class, sidebar CSS class and visibility of sidebar.
	 * return array
	 */
	function human_consult_get_columns_classes( $full_width = false ) {
		//default values
		$column_classes = array(
			'main_column_class' => 'col-xs-12 col-sm-7 col-md-8 col-lg-8',
		...
		if ( function_exists( 'fw_ext_sidebars_get_current_position' ) ) {
			...
			//no catching right sidebar. Right sidebar is default
				...
				if ( is_page() ) {
					$column_classes['main_column_class'] = "col-xs-12 col-sm-12";
					$column_classes['sidebar_class']     = false;
				}
		...
		if ( $full_width || !human_consult_is_active_widgets_in_main_sidebar_exists() ) {
			$column_classes['main_column_class'] = "col-xs-12 col-sm-12";
			$column_classes['sidebar_class']     = false;
		}
	...
	} //human_consult_get_columns_classes()

endif;

if ( ! function_exists( 'human_consult_is_active_widgets_in_main_sidebar_exists' ) ) :
	/**
	 * Define is sidebar that must be shown has active widgets
	 */
	function human_consult_is_active_widgets_in_main_sidebar_exists() {
		//default value
		$return = true;
...
if ( ! function_exists( 'human_consult_get_columns_classes_for_unyson_extended' ) ) :
	/**
	 * Define a sidebar position for manage main column CSS class, sidebar CSS class and visibility of sidebar.
	 * return array
	 */
	function human_consult_get_columns_classes_for_unyson_extended( $full_width = false ) {

		// Sidebar Position

		// default
		$sidebar_position = apply_filters( 'human_consult_default_sidebar_position', 'right' );
		...
		$empty = false;
		if ( apply_filters( 'human_consult_check_for_widgets', true ) ) {
		...
		$s = apply_filters( 'human_consult_sidebar_width', 4 );             // sidebar width
 ...
 */
if ( ! function_exists( 'human_consult_bootstrap_paginate_links' ) ) {
	function human_consult_bootstrap_paginate_links( $args = '' ) {
		...
		$defaults = array(
			...
			'show_all'           => false,
			'prev_next'          => true,
			'prev_text'          => esc_html__( 'Prev', 'human-consult' ),
			'next_text'          => esc_html__( 'Next', 'human-consult' ),
			'end_size'           => 1,
			'mid_size'           => 2,
			'type'               => 'plain',
			'add_args'           => array(),
		...
		return $r;
	} //human_consult_bootstrap_paginate_links()
}

if ( ! function_exists( 'human_consult_paging_nav' ) ) :
	/**
	 * Display navigation to next/previous set of posts when applicable.
	 */
	function human_consult_paging_nav( $wp_query = null, $wrapper = null ) {
		...
		$links = human_consult_bootstrap_paginate_links( array(
			'base'      => $pagenum_link,
			'format'    => $format,
			'show_all'  => false,
	...
	} //human_consult_paging_nav()

endif;

if ( ! function_exists( 'human_consult_paging_comments_nav ' ) ) :
	/**
	 * Display navigation to next/previous set of posts when applicable.
	 */
	function human_consult_paging_comments_nav( $args = array() ) {
		...
		$args       = wp_parse_args( $args, $defaults );
		$page_links = human_consult_bootstrap_paginate_links( $args );
	...
	} //human_consult_paging_comments_nav()
 ...
 */
if ( ! function_exists( 'human_consult_categorized_blog' ) ) :
	function human_consult_categorized_blog() {
		if ( false === ( $all_categories = get_transient( 'human_consult_category_count' ) ) ) {
			...
			set_transient( 'human_consult_category_count', $all_categories );
		}

		if ( 1 !== (int) $all_categories ) {
			// This blog has more than 1 category so human_consult_categorized_blog should return true
			return true;
		} else {
			// This blog has only 1 category so human_consult_categorized_blog should return false
			return false;
		}
	} //human_consult_categorized_blog()
endif;

if ( ! function_exists( 'human_consult_post_nav' ) ) :
	/**
	 * Display navigation to next/previous post when applicable.
	 */
	function human_consult_post_nav() {
		...
		<nav class="navigation post-navigation items-nav greylinks" role="navigation">
			...
			?>
			<?php previous_post_link( '%link', '<div class="media with_background  prev-item text-center" style="background-image: url(' . esc_url( $prev_thumbnail ) . '); "><div class="nav-overlay"></div><div class="nav-middle"><span class="nav">' . esc_html__( 'Prev', 'human-consult' ) . '</span><span class="title">%title</span></div></div>' ); ?>
			<?php next_post_link( '%link', '<div class="media with_background next-item text-center" style="background-image: url(' . esc_url( $next_thumbnail ) . '); "><div class="nav-overlay"></div><div class="nav-middle"><span class="nav">' . esc_html__( 'Next', 'human-consult' ) . '</span><span class="title">%title</span></div></div>' ); ?>
		</nav><!-- .navigation -->
	<?php } //human_consult_post_nav
endif;

if ( ! function_exists( 'human_consult_posted_on' ) ) : /**
 * Print HTML with meta information for the current post-date/time and author.
 */
	function human_consult_posted_on() {
...
endif; //human_consult_posted_on

if ( ! function_exists( 'human_consult_posted_by' ) ) : /**
 * Print HTML with meta information for the current post-date/time and author.
 */
	function human_consult_posted_by() {
		if ( is_sticky() && is_home() && ! is_paged() ) {
			echo '<span class="featured-post"><i class="rt-icon2-clip highlight"></i>' . esc_html__( ' Sticky: ', 'human-consult' ) . '</span>';
		}
 ...
 */
if ( ! function_exists( 'human_consult_post_thumbnail' ) ) :
	function human_consult_post_thumbnail( $small_image = false ) {
		$pID = get_the_ID();
		...
		if ( get_post_format( $pID ) == 'gallery' ) {
			...
			human_consult_shortcode_atts_gallery_trigger( false );
			...
			<div
					...
					<div class="entry-meta-corner">
						...
						if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) :
							...
							<span class="comments-link">
								...
								<?php comments_popup_link( esc_html__( '0', 'human-consult' ), esc_html__( '1', 'human-consult' ), esc_html__( '%', 'human-consult' ) ); ?>
						</span>
							<?php
				...
				if ( ! $is_gallery ) :
					...
					if (
						...
						the_post_thumbnail( 'human-consult-full-width' );
					} elseif ( ! is_singular() && $small_image ) {
						the_post_thumbnail( 'human-consult-small-width' );
					} else {
						the_post_thumbnail();
					} //$current_position
...
// share buttons
if ( ! function_exists( 'human_consult_share_this' ) ) :
	/**
	 * Share article through social networks.
	 * bool $only_buttons
	 */
	function human_consult_share_this( $only_buttons = false ) {
	...
	} //human_consult_share_this()
endif; //function_exists

//get predefined template part from theme options
if ( ! function_exists( 'human_consult_get_predefined_template_part' ) ) :
	/**
	 * Return propper template part from options or default.
	 * string $template_part_name
	 */
	function human_consult_get_predefined_template_part( $template_part_name ) {
	...
	} //human_consult_get_predefined_template_part()

endif;//function_exists

//get ids of showing widgets
if ( ! function_exists( 'human_consult_get_showing_widgets_ids' ) ) :
	/**
	 * Return array of id's of all widgets that are showing.
	 */

	function human_consult_get_showing_widgets_ids() {
...
if ( ! function_exists( 'human_consult_get_posts_single_taxonomy_name' ) ) :
	function human_consult_get_posts_single_taxonomy_name( $queried_object ) {
...
if ( ! function_exists( 'human_consult_get_post_categories' ) ) :
	function human_consult_get_post_categories( $taxonomy_name = 'category' ) {
...
if ( ! function_exists( 'human_consult_get_categories_slugs_for_single_post' ) ) :
	function human_consult_get_categories_slugs_for_single_post( $taxonomy_name = 'category' ) {
