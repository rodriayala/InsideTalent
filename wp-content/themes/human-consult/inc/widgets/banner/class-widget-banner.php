...
if ( ( defined( 'FW' ) ) && ! ( class_exists( 'Human_Consult_Widget_Banner' ) ) ) :

class Human_Consult_Widget_Banner extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			...
			'description' => esc_html__( 'Add linked image with text', 'human-consult' ),
		);

		parent::__construct( false, esc_html__( 'Theme - Ad Banner', 'human-consult' ), $widget_ops );
		...
		$this->options = array(
			...
			'image' => array(
				'type'        => 'upload',
				'value'       => '',
				'label'       => esc_html__( 'Banner Image', 'human-consult' ),
				'images_only' => true,
			),
			'url'   => array(
				'type'  => 'text',
				'value' => '',
				'label' => esc_html__( 'Banner URL', 'human-consult' ),
			),
