...
if ( ! function_exists( 'human_consult_auto_taxonomy' ) ) :
	/**
	 * Return taxonomy of given type for given post_type.
	 */
	function human_consult_auto_taxonomy( $args = array() ) {
		...
		$taxonomies = array_merge( $taxonomies, apply_filters( 'human_consult_auto_taxonomy_list', array() ) );
...
if ( ! function_exists( 'human_consult_tt_kses_list' ) ) :
	/**
	 * Return array of allowed tags for template tags functions.
	 */
	function human_consult_tt_kses_list( $args = array() ) {

		if ( array_key_exists( 'kses_list', $args ) ) {
			$out = $args['kses_list'];
		} else {
			$out = function_exists( 'human_consult_kses_list' ) ? human_consult_kses_list() : 'post';
			$out = apply_filters( 'human_consult_tt_kses_list', $out );
		}
...
if ( ! function_exists( 'human_consult_get_the_excerpt' ) ) :
	/**
	 * Retrieve excerpt with custom length in characters or words and custom more link.
	 */
	function human_consult_get_the_excerpt( $args = array() ) {
		...
		$defaults = array_merge( $defaults, apply_filters( 'human_consult_get_the_excerpt_defaults', array() ) );
		...
		if ( ! empty( $post ) ) {
			...
			if ( post_password_required( $post ) ) {
				...
				$out = esc_html__( 'There is no excerpt because this is a protected post.', 'human-consult' );
			} else {
		...
		return apply_filters( 'human_consult_get_the_excerpt', $out );
	}
endif;


if ( ! function_exists( 'human_consult_the_excerpt' ) ) :
	/**
	 * Echo excerpt with custom length in characters or words and custom more link.
	 */
	function human_consult_the_excerpt( $args = array() ) {

		echo wp_kses( human_consult_get_the_excerpt( $args ), human_consult_tt_kses_list( $args ) );
	}
endif;
...
if ( ! function_exists( 'human_consult_get_term_choices' ) ) :
	/**
	 * Retrieve associative array of "term_id => term_name" pairs.
	 */
	function human_consult_get_term_choices( $post_id, $taxonomy ) {
...
if ( ! function_exists( 'human_consult_get_the_terms' ) ) :
	/**
	 * Retrieve formatted terms.
	 */
	function human_consult_get_the_terms( $args = array() ) {
		...
		$defaults = array(
			...
			'items_separator' => '<span>' . esc_html_x(
					', ', 'Used between list items, there is a space after the comma.', 'human-consult' ) . '</span>',
		...
		$defaults = array_merge( $defaults, apply_filters( 'human_consult_get_the_terms_defaults', array() ) );
...
//		                $ellipses_length = strlen( $ellipsis_text );
		...
		return apply_filters( 'human_consult_get_the_terms', $out );
	}
endif;


if ( ! function_exists( 'human_consult_the_terms' ) ) :
	/**
	 * Echo formatted terms.
	 */
	function human_consult_the_terms( $args = array() ) {

		echo wp_kses( human_consult_get_the_terms( $args ), human_consult_tt_kses_list( $args ) );
	}
endif;


if ( ! function_exists( 'human_consult_get_featured_term' ) ) :
	/**
	 * Retrieve featured term.
	 */
	function human_consult_get_featured_term( $args = array() ) {

		if ( ! array_key_exists( 'max_items', $args ) ) {
			$args['max_items'] = 1;
		}

		return human_consult_get_the_terms( $args );
	}
endif;


if ( ! function_exists( 'human_consult_featured_term' ) ) :
	/**
	 * Echo featured term.
	 */
	function human_consult_featured_term( $args = array() ) {

		echo wp_kses( human_consult_get_featured_term( $args ), human_consult_tt_kses_list( $args ) );
	}
endif;


if ( ! function_exists( 'human_consult_get_the_categories' ) ) :
	/**
	 * Retrieve formatted categories.
	 */
	function human_consult_get_the_categories( $args = array() ) {

		if ( ! array_key_exists( 'taxonomy', $args ) ) {
			$args['taxonomy'] = 'category';

			if ( apply_filters( 'human_consult_use_auto_taxonomy_for_template_tags', true ) ) {
				$args['taxonomy'] = human_consult_auto_taxonomy( array( 'type' => 'category' ) );
			}
		}
...
if ( ! function_exists( 'human_consult_the_categories' ) ) :
	/**
	 * Echo formatted categories.
	 */
	function human_consult_the_categories( $args = array() ) {

		echo wp_kses( human_consult_get_the_categories( $args ), human_consult_tt_kses_list( $args ) );
	}
endif;


if ( ! function_exists( 'human_consult_get_featured_category' ) ) :
	/**
	 * Retrieve featured category.
	 */
	function human_consult_get_featured_category( $args = array() ) {
		...
		return human_consult_get_the_categories( $args );
	}
endif;


if ( ! function_exists( 'human_consult_featured_category' ) ) :
	/**
	 * Echo featured category.
	 */
	function human_consult_featured_category( $args = array() ) {

		echo wp_kses( human_consult_get_featured_category( $args ), human_consult_tt_kses_list( $args ) );
	}
endif;


if ( ! function_exists( 'human_consult_get_the_tags' ) ) :
	/**
	 * Retrieve formatted tags.
	 */
	function human_consult_get_the_tags( $args = array() ) {

		if ( ! array_key_exists( 'taxonomy', $args ) ) {
			$args['taxonomy'] = 'post_tag';

			if ( apply_filters( 'human_consult_use_auto_taxonomy_for_template_tags', true ) ) {
				$args['taxonomy'] = human_consult_auto_taxonomy( array( 'type' => 'tag' ) );
			}
		}
...
if ( ! function_exists( 'human_consult_the_tags' ) ) :
	/**
	 * Echo formatted tags.
	 */
	function human_consult_the_tags( $args = array() ) {

		echo wp_kses( human_consult_get_the_tags( $args ), human_consult_tt_kses_list( $args ) );
	}
endif;


if ( ! function_exists( 'human_consult_get_featured_tag' ) ) :
	/**
	 * Retrieve featured tag.
	 */
	function human_consult_get_featured_tag( $args = array() ) {
		...
		return human_consult_get_the_tags( $args );
	}
endif;


if ( ! function_exists( 'human_consult_featured_tag' ) ) :
	/**
	 * Echo featured tag.
	 */
	function human_consult_featured_tag( $args = array() ) {

		echo wp_kses( human_consult_get_featured_tag( $args ), human_consult_tt_kses_list( $args ) );
	}
endif;
...
if ( ! function_exists( 'human_consult_get_object_time' ) ) :
	/**
	 * Retrieve formatted object time.
	 */
	function human_consult_get_object_time( $args = array() ) {
		...
		$defaults = array(
			...
			'days_ago'          => false,
			'days_ago_max_days' => - 1,
			'days_ago_text'     => esc_html__( 'days ago', 'human-consult' ),
			'today'             => esc_html__( 'Today', 'human-consult' ),
			'yesterday'         => esc_html__( 'Yesterday', 'human-consult' ),
			'week_ago'          => esc_html__( 'Week ago', 'human-consult' ),
			'weeks_ago'         => esc_html__( 'weeks ago', 'human-consult' ),
			'month_ago'         => esc_html__( 'Month ago', 'human-consult' ),
			'months_ago'        => esc_html__( 'months ago', 'human-consult' ),
			'year_ago'          => esc_html__( 'Year ago', 'human-consult' ),
			'years_ago'         => esc_html__( 'years ago', 'human-consult' ),
		);

		$defaults = array_merge( $defaults, apply_filters( 'human_consult_get_object_time_defaults', array() ) );
		...
		return apply_filters( 'human_consult_get_object_time', $out );
	}
endif;


if ( ! function_exists( 'human_consult_get_the_date' ) ) :
	/**
	 * Retrieve formatted post date.
	 */
	function human_consult_get_the_date( $args = array() ) {
		...
		return human_consult_get_object_time( $args );
	}
endif;


if ( ! function_exists( 'human_consult_the_date' ) ) :
	/**
	 * Echo formatted post date.
	 */
	function human_consult_the_date( $args = array() ) {

		echo wp_kses( human_consult_get_the_date( $args ), human_consult_tt_kses_list( $args ) );
	}
endif;


if ( ! function_exists( 'human_consult_get_the_time' ) ) :
	/**
	 * Retrieve formatted post time.
	 */
	function human_consult_get_the_time( $args = array() ) {
...
if ( ! function_exists( 'human_consult_the_time' ) ) :
	/**
	 * Echo formatted post time.
	 */
	function human_consult_the_time( $args = array() ) {

		echo wp_kses( human_consult_get_the_time( $args ), human_consult_tt_kses_list( $args ) );
	}
endif;


if ( ! function_exists( 'human_consult_get_comment_date' ) ) :
	/**
	 * Retrieve formatted comment date.
	 */
	function human_consult_get_comment_date( $args = array() ) {
...
if ( ! function_exists( 'human_consult_comment_date' ) ) :
	/**
	 * Echo formatted comment date.
	 */
	function human_consult_comment_date( $args = array() ) {

		echo wp_kses( human_consult_get_comment_date( $args ), human_consult_tt_kses_list( $args ) );
	}
endif;


if ( ! function_exists( 'human_consult_get_comment_time' ) ) :
	/**
	 * Retrieve formatted comment time.
	 */
	function human_consult_get_comment_time( $args = array() ) {
...
if ( ! function_exists( 'human_consult_comment_time' ) ) :
	/**
	 * Echo formatted comment time.
	 */
	function human_consult_comment_time( $args = array() ) {

		echo wp_kses( human_consult_get_comment_time( $args ), human_consult_tt_kses_list( $args ) );
	}
endif;
...
if ( ! function_exists( 'human_consult_get_comments_counter' ) ) :
	/**
	 * Retrieve formatted post comments counter.
	 */
	function human_consult_get_comments_counter( $args = array() ) {
		...
		$defaults = array_merge( $defaults, apply_filters( 'human_consult_get_comments_counter_defaults', array() ) );
		...
		return apply_filters( 'human_consult_get_comments_counter', $out );
	}
endif;


if ( ! function_exists( 'human_consult_comments_counter' ) ) :
	/**
	 * Echo formatted post comments counter.
	 */
	function human_consult_comments_counter( $args = array() ) {

		echo wp_kses( human_consult_get_comments_counter( $args ), human_consult_tt_kses_list( $args ) );
	}
endif;
...
if ( ! function_exists( 'human_consult_get_the_author' ) ) :
	/**
	 * Retrieve formatted author vcard.
	 */
	function human_consult_get_the_author( $args = array() ) {
		...
		$defaults = array_merge( $defaults, apply_filters( 'human_consult_get_the_author_defaults', array() ) );
		...
		return apply_filters( 'human_consult_get_the_author', $out );
	}
endif;


if ( ! function_exists( 'human_consult_the_author' ) ) :
	/**
	 * Echo formatted author vcard.
	 */
	function human_consult_the_author( $args = array() ) {

		echo wp_kses( human_consult_get_the_author( $args ), human_consult_tt_kses_list( $args ) );
	}
endif;
...
if ( ! function_exists( 'human_consult_get_sticky_marker' ) ) :
	/**
	 * Retrieve formatted sticky marker.
	 */
	function human_consult_get_sticky_marker( $args = array() ) {
		...
		$defaults = array_merge( $defaults, apply_filters( 'human_consult_get_sticky_marker_defaults', array() ) );
		...
		return apply_filters( 'human_consult_get_sticky_marker', $out );
	}
endif;


if ( ! function_exists( 'human_consult_sticky_marker' ) ) :
	/**
	 * Echo formatted sticky marker.
	 */
	function human_consult_sticky_marker( $args = array() ) {

		echo wp_kses( human_consult_get_sticky_marker( $args ), human_consult_tt_kses_list( $args ) );
	}
endif;